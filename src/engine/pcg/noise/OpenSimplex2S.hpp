#ifndef OPENSIMPLEX2S_HPP
#define OPENSIMPLEX2S_HPP
 ...
 * Also changed some code to use fixed c-style arrays, to avoid using of std
 * library.
 */
...
class OpenSimplex2S {
  ...
  NoiseResult2D noise2_Base_deriv(double xs, double ys);
...
public:
  ...
  /**
   * Expose derivative values
   */
  void noise2_deriv(double x, double y, double& out_value, double& out_dx,
                    double& out_dy);
   ...
   * case. If Y is vertical in world coordinates, call noise3_XYBeforeZ(x, z, Y)
   * or use noise3_XZBeforeY. If Z is vertical in world coordinates, call
   ...
   * Recommended for 3D terrain and time-varied animations.
   * The Y coordinate should always be the "different" coordinate in your use
   * case. If Y is vertical in world coordinates, call noise3_XZBeforeY(x, Y,
   * z). If Z is vertical in world coordinates, call noise3_XZBeforeY(x, Z, y)
   * or use noise3_XYBeforeZ. For a time varied animation, call
   * noise3_XZBeforeY(x, T, y) or use noise3_XYBeforeZ.
   */
